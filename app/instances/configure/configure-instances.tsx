    ...
    async function handleCreateInstance(event: FormEvent<HTMLFormElement>) {
        ...
        try {
            const result = await createInstance({
                name: trimmedName,
                splitCode: "", // generated by MyIMO Pay — placeholder satisfies the TS type
                idclPercent: 0,
    ...
    return (
        <>
                        ...
                        <div className="flex flex-col items-center space-y-5">
                            ...
                            <div className="text-center space-y-2">
                                ...
                                <p className="text-sm text-[var(--muted-foreground)] max-w-xs">
                                    Setting up payment configuration with MyIMO Pay...
                                </p>
                            </div>
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                    <Link href="/instances" className="text-sm font-medium text-[var(--accent)]">
                ...
                <article className="rounded-2xl border border-[var(--border)] bg-[var(--surface-soft)] p-4">
                    ...
                    <form className="mt-4 grid gap-3" onSubmit={handleCreateInstance}>
                        ...
                        <div className="rounded-xl border-2 border-dashed border-[var(--border)] p-3">
                            <div className="flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        const existing = formFields.map((f) => f.key.toLowerCase());
                                        if (reserved.some((r) => existing.includes(r)) || formFields.some((f) => reserved.includes(f.key.toLowerCase()))) {
                                            return;
                                        }
                            ...
                            <p className="mt-1 text-xs text-[var(--muted-foreground)]">
                                Define what information operators must capture per payment (e.g. vehicle plate, address).
                            </p>

                            {/* All fields: native + custom */}
                            <div className="mt-3 space-y-2">
                                {/* Native fields (read-only) */}
                                <div className="flex items-center gap-2 rounded-lg bg-[var(--surface-alt)] px-3 py-2.5 opacity-70">
                                    <div className="flex flex-1 items-center gap-2">
                                        <span className="text-xs font-medium text-[var(--muted-foreground)] w-16">Text</span>
                                        <span className="text-sm text-[var(--foreground)]">Full name</span>
                                    </div>
                                    <span className="rounded-full bg-[var(--surface)] border border-[var(--border)] px-2 py-0.5 text-[10px] font-medium text-[var(--muted-foreground)]">
                                        Built-in
                                    </span>
                                </div>
                                <div className="flex items-center gap-2 rounded-lg bg-[var(--surface-alt)] px-3 py-2.5 opacity-70">
                                    <div className="flex flex-1 items-center gap-2">
                                        <span className="text-xs font-medium text-[var(--muted-foreground)] w-16">Email</span>
                                ...
                                {formFields.map((field, index) => {
                                    ...
                                    return (
                                    <div key={index} className={`space-y-2 rounded-lg p-2 ${isReserved ? "bg-[var(--danger)]/5 border border-[var(--danger)]/30" : "bg-[var(--surface)]"}`}>
                                        {isReserved && (
                                            <p className="text-xs text-[var(--danger)] font-medium">
                                                &quot;{field.key}&quot; is a built-in field. Please use a different label.
                                            </p>
                                        )}
                                        <div className="flex gap-2">
                                            <input
                                ...
                                {formFields.length === 0 && (
                                    <p className="py-2 text-center text-xs text-[var(--muted-foreground)]">
                                        No custom fields added. Only built-in name and email fields will be shown.
                                    </p>
                                )}
                            </div>
                        </div>
